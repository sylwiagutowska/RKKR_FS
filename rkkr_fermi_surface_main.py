...
class init_calc:
    def __init__(self):
        ...
        self.file_kgrid='kvec.dat'
#        self.COMMAND = 'rm fileo*;/home/sylwia/bin/RKKR/rkkr-hop/rkkr004-gf >'+self.file_out
#        self.COMMAND = 'rm fileo*;/home/sylwia/bin/RKKR/SRC_2017/Rcpa005/rcpa005 >'+self.file_out
        self.COMMAND = 'rm fileo*;/home/software/kkr/rcpa005-rh >'+self.file_out
...
class Obiekt(init_calc):
    ...
    def make_k_grid(self):
      if self.COMMAND=='': 
          self.VEC,self.VEC_all,self.VEC_weights=\
          func.read_kgrid_from_file(self.file_kgrid)
      else:
    ...
    def ene_from_irreducible_to_whole_grid(self):
        ...
        self.ENE = func.ene_from_irreducible_to_whole_grid(
                 self.ENE,self.VEC,self.VEC_all,self.nkp, self.nbnd)
        print ("...Done.")
    def fix_bad_values(self):
        print("...I am fixing incorrect values...")
        prm=0.25/13.606
        self.ENE=func.fix_bad_values(min(self.nbnd),self.nkp,self.ENE,prm)
#        self.ENE=func.fix_bad_values2(min(self.nbnd),self.nkp,self.ENE,prm)
...
#3.a.print to file
h=open(obj.file_kgrid,'w')
for ni,i in enumerate(obj.VEC_all):
 for j in i:
  h.write(str(j)+' ')
 h.write(str(obj.VEC_weights[i[3]])+' \n')
h.close()
