...
b_vec=[ b_vec[m]/2./np.pi*alat[m] for m in range(3)]
print b_vec
VEC=[]
nkp=24
for i in range(nkp):
 for j in range(nkp):
  for k in range(2):
   v=np.transpose(np.array([i/float(nkp),j/float(nkp),k]))
...
h.close()
os.system('rm fileo*;~/programy/RKKR/rkkr-hop/rkkr004-gf >out')
print('calc done')
#####
...
h=open('fileo9')
tmp=[i.split() for i in h.readlines()[1:]]
h.close()
...
for line in tmp[1:]:
 if len(line)==4: 
  ...
  ENE[-1][-1].append(float(line[1])-fermi)
...
nband=min(nbnd)
ENE=np.array([ [ [ j[nk] for nk in range(len(j)) if nk<nbnd] for j in i] for i in ENE])
