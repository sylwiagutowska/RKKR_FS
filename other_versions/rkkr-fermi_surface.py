...
if 'rcpa' in COMMAND: liczba=4
 ...
 liczba=4
 try: liczba=int(raw_input('Rkkr (3)  or Rcpa (4)? Type 3 or 4 [4]: '))
 except: liczba=int(input('Rkkr (3)  or Rcpa (4)? Type 3 or 4 [4]: '))
for nl,line in enumerate(tmp[1:]):
 if len(line)==4 and '      ' in tmp0[nl+1]: 
  ...
  ENE[-1][-1].append(float(line[1].replace('D','E'))-fermi)
